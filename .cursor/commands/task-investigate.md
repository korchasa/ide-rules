...
1. **Intake**
   ...
   - Gather missing critical data (repo/path, OS/version/arch, revision, deps, exact repro, expected vs actual, logs/traces, time/resource limits)
   - Confirm environment boundaries

2. **Hypotheses (Sample-then-Filter)**
   - Propose 5–10 candidate root causes with brief evidence/probe
   - Drop hypotheses incompatible with facts/logs/repro
   - Assign coarse probabilities and normalize

3. **Design experiments (rank by EIG)**
   - Generate 3–5 diverse experiments (targeted tests/logs, config/version isolation, git bisect, last-diff inspection, static analysis, focused web search)
   - Define discrete outcomes for each experiment
   - Estimate EIG; pick max-EIG experiment and request approval

4. **Execute approved experiment**
   - If diagnostic edits are needed, keep them minimal and isolated
   - Run, collect outcomes and artifacts, bucketize results

5. **Update beliefs and decide**
   - Filter contradicted hypotheses; renormalize probabilities; maintain a Hypothesis Board
   - Branching:
     - If strong support (≥85%) and verifiable fix exists — request approval to proceed to fix
     - If weakened/falsified — roll back diagnostic changes and design new experiments
     - Otherwise — prepare next experiment set (ranked by EIG)
